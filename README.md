...
學術核心貢獻：可遷移的 LLM 行為評估方法論：三層漸進式框架（穩定性 → 行為模式 → 因子探索），以探索性資料分析取代假設檢驗；結合非監督式學習（K-means／PCA）、Bootstrap BCa 與效應量處理小樣本與高隨機性。核心邏輯「先立穩定性、再探行為」可移轉至其他無標準答案的 LLM 系統——並以誠實報告負面結果展現方法論紀律。
